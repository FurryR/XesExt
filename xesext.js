 ...
 * 此程序基于 GPL-3.0 开源。
 */
// 注，如果你需要连字特性，请在自定义加载脚本的位置添加以下行：
// https://cdn.jsdelivr.net/npm/xterm/lib/xterm.min.js
// 然后检查实现部分，并取消注释加载连字特性的代码。
...
;(function () {
      ...
      set(e) {
        if (newref == undefined) {
          ...
          if (xterm) {
            ...
            // 删除本地作品栏
            if (
              xterm.parentNode.parentNode.parentNode.parentNode.classList.contains(
                'offline'
              )
            ) {
              xterm.parentNode.parentNode.parentNode.parentNode.classList.remove(
                'offline'
              )
              xterm.parentNode.parentNode.parentNode.parentNode.parentNode.childNodes[1].classList.remove(
                'offline'
              )
  ...
  window.setTimeout = (code, delay, ...args) => {
    if (
      code.toString().indexOf('fnTryLockRun') != -1 ||
      code
        .toString()
        .indexOf("document.getElementById('loading-dom').style.display") != -1
    ) {
      code()
      return -1
    }
