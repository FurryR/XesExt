...
let SPAM_FILTER = (data) => data.name.indexOf("随堂") != -1
// [审查用]取得直链
...
;(function () {
    ...
    if (project) {
        ...
        window.XMLHttpRequest.prototype.open = function (e, t, n) {
            ...
            if (
                ...
                if (tmp.status != 200) {
                    ...
                    tmp.__xes_disableFilter = true
                    tmp.open(e, this.__xes_url = `/api/community/v4/projects/detail?id=${project[0]}&lang=${project[2]}`, false)
                    tmp.send()
                }
        ...
        window.XMLHttpRequest.prototype.open = function (e, t, n) {
            this.__xes_url = t
            if (
                (t.startsWith('/api/works/latest') || t.startsWith('/api/works/popular') || t.startsWith('/api/works/courses')) && !this.__xes_disableFilter) {
